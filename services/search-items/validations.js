var verifyStatusCode = function (res, counter, actionName) {
  ...
  if (!statusOk) console.error('Search item request has status code: ', res.status);
  return statusOk;
};
...
var verifyCorrectResponseDataLoadTest = function (res, searchItemString, counter, actionName) {
  ...
  if (!correctResponse) console.error('Search item request has response data: ', res.body);
